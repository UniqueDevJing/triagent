  ...
  <div class="page-container">
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑宣教内容' : '新增宣教内容'" width="700px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="类型"><el-select v-model="form.type"><el-option v-for="(v,k) in typeMap" :key="k" :label="v" :value="k" /></el-select></el-form-item>
...
const dialogVisible = ref(false), form = ref({ type: 'ARTICLE' })
const previewVisible = ref(false), previewContent = ref('')

onMounted(() => fetch())
async function fetch() {
  loading.value = true
  const res = await api.getEducationContents({ page: page.value, size: size.value, keyword: keyword.value, type: typeFilter.value })
  tableData.value = res.data.records; total.value = res.data.total; loading.value = false
}
function openDialog(row) { form.value = row ? { ...row } : { type: 'ARTICLE' }; dialogVisible.value = true }
async function save() {
