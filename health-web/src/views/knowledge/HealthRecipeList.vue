<template>
  <div class="page-container">
    ...
    <el-table :data="tableData" v-loading="loading" stripe>
      <template #empty>
        <EmptyState title="暂无食谱" description="创建健康食谱库，为会员提供科学的饮食指导" icon="Dish" action-text="新增食谱" @action="openDialog()" />
      </template>
      <el-table-column prop="recipeName" label="名称" width="140" />
      <el-table-column prop="mealType" label="餐型" width="100">
        <template #default="{ row }"><el-tag size="small">{{ catMap[row.mealType] || row.mealType }}</el-tag></template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑食谱' : '新增食谱'" width="700px">
      <el-form :model="form" label-width="80px">
        <el-form-item label="名称" required><el-input v-model="form.recipeName" /></el-form-item>
        <el-form-item label="餐型"><el-select v-model="form.mealType"><el-option v-for="(v,k) in catMap" :key="k" :label="v" :value="k" /></el-select></el-form-item>
        <el-form-item label="食材清单"><el-input v-model="form.ingredients" type="textarea" :rows="3" /></el-form-item>
        <el-form-item label="制作步骤"><el-input v-model="form.steps" type="textarea" :rows="4" /></el-form-item>
        <el-form-item label="总热量"><el-input-number v-model="form.totalCalories" :min="0" /> kcal</el-form-item>
...
const catMap = { BREAKFAST: '早餐', LUNCH: '午餐', DINNER: '晚餐', SNACK: '加餐/甜品' }
const diffMap = { EASY: '简单', MEDIUM: '中等', HARD: '困难' }
const tableData = ref([])
const loading = ref(false)
const page = ref(1), size = ref(20), total = ref(0)
const keyword = ref(''), catFilter = ref('')
const dialogVisible = ref(false), form = ref({ mealType: 'BREAKFAST', difficulty: 'EASY' })
...
function openDialog(row) { form.value = row ? { ...row } : { mealType: 'BREAKFAST', difficulty: 'EASY' }; dialogVisible.value = true }
async function save() {
